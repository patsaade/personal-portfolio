...
const mitigationUrlById = {}; // course-of-action id → its own MITRE mitigation page URL
for (const o of objects) {
  if (o.name) nameById[o.id] = o.name;
  if (o.type === 'x-mitre-data-component') dcName[o.id] = o.name;
  if (o.type === 'course-of-action' && !o.revoked) { const r = ref(o); if (r) mitigationUrlById[o.id] = r.url; }
}
...
for (const o of objects) {
  ...
  if (o.relationship_type === 'uses' && o.description && nameById[o.source_ref]) {
    ...
    (mitigations[t] ??= []).push({ name: nameById[o.source_ref] || '', text: trimTo(o.description, 180), url: mitigationUrlById[o.source_ref] });
...
for (const id of [...wanted].sort()) {
  ...
  const mit = (mitigations[t.stix] || [])
    ...
    .map((m) => (m.url ? { name: m.name, text: m.text, url: m.url } : { name: m.name, text: m.text }));
